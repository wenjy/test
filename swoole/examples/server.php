...
//        'task_ipc_mode'            => 2,     // 设置 Task 进程与 Worker 进程之间通信的方式。
    ...
    /**
     * @param $fd
     * @param $create bool 如果buffer不存在，是否创建
     * @return swoole_buffer|bool
     */
...
if ($res = $serv->listen('0.0.0.0', 9502, SWOOLE_SOCK_UDP)) {
    echo 'listen->0.0.0.0:9502 UDP success' .  PHP_EOL;
} else {
    echo 'listen->0.0.0.0:9502 UDP fail error->' .  $serv->getLastError() . PHP_EOL;
}

if ($res = $serv->listen('::', 9503, SWOOLE_SOCK_TCP6)) {
    echo 'listen->:::9503 TCP6 success' .  PHP_EOL;
} else {
    echo 'listen->:::9503 TCP6 fail error->' .  $serv->getLastError() . PHP_EOL;
}

if ($res = $serv->listen('::', 9504, SWOOLE_SOCK_UDP6)) {
    echo 'listen->:::9504 UDP6 success' .  PHP_EOL;
} else {
    echo 'listen->:::9504 UDP6 fail error->' .  $serv->getLastError() . PHP_EOL;
}

// 使用 swoole_process 替代PHP的pcntl
